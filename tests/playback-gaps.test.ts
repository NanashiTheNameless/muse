...
const makeAutocompleteHarness = (query: string) => {
  const cache = {
  ...
  const command = new Play(cache as never, {} as never, {INSTANCE_OWNER_ID: ''} as never);
...
describe('PLAY-14 yt-dlp unavailable classification preservation', () => {
  it.each([
    ...
    await expect(getYouTubeMediaSource('abcdefghijk')).rejects.toMatchObject({
      ...
      reason,
      message: expect.stringContaining(`ERROR: ${detail}`) as string,
    });
  });
  ...
  it('surfaces a general extraction failure without misclassifying it as unavailable', async () => {
    ...
    try {
      ...
      expect((error as Error).message).toContain('yt-dlp failed to extract media:');
      expect((error as Error).message).toContain('ERROR: upstream TLS handshake failed');
    }
  });
});
...
describe('CTRL-20 audio-idle advance preservation', () => {
  ...
  it('announces only the natural idle advance, not a manual forward', async () => {
    ...
    // Play the track to near its end so the idle reads as natural completion
    // rather than an unexpected early drop (which the player retries instead).
  ...
  it('restarts the same entry from zero when song loop is enabled', async () => {
    ...
    await vi.advanceTimersByTimeAsync(96_000);
    expect(player.getPosition()).toBe(96);
