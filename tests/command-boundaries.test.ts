...
const makeQueuePlayer = (upcomingCount: number) => {
  ...
  return {
    ...
    getOverriddenTarget: () => null,
    getPreDuckingVolume: () => null,
  };
};

const fakeConfigService = () => ({INSTANCE_OWNER_ID: ''});
...
const makeInteraction = (options: {
  ...
  const interaction = {
    ...
    user: {id: 'requester'},
    member: {
      permissions: {has: () => options.hasManageGuild ?? true},
      voice: {channel: null},
    },
    options: {
...
const makeCommands = () => {
  ...
  return [
    ...
    new Config(fakeConfigService() as never),
    ...
    new Play(undefined as never, addQueryToQueue, fakeConfigService() as never),
    new Queue(playerManager),
    new Remove(playerManager, fakeConfigService() as never),
    new Replay(playerManager),
    new Resume(playerManager),
    new Seek(playerManager),
    new Shuffle(playerManager),
    new Skip(playerManager, fakeConfigService() as never),
    new Stop(playerManager),
    new Unskip(playerManager),
    new Volume(playerManager, fakeConfigService() as never),
  ];
};
...
describe('command metadata', () => {
  ...
  it('preserves the audited voice-channel requirement matrix', () => {
    ...
    const neverRequiresVoice = ['config', 'move', 'now-playing', 'queue', 'remove', 'volume'];
  ...
  it('publishes the existing audited integer option bounds', () => {
    ...
    expect(findOption(serialized.get('volume')!, 'set', 'level')).toMatchObject({min_value: 0, max_value: 100});
    expect(findOption(serialized.get('config')!, 'set-wait-after-queue-empties', 'delay')).toMatchObject({min_value: 0});
    expect(findOption(serialized.get('config')!, 'set-duck-target', 'volume')).toMatchObject({min_value: 0, max_value: 100});
  ...
  it('gates /config at runtime instead of default member permissions', async () => {
    const config = new Config(fakeConfigService() as never).slashCommand.toJSON();

    expect(config.default_member_permissions).toBeUndefined();

    const {interaction, reply} = makeInteraction({subcommand: 'get', hasManageGuild: false});
    await new Config(fakeConfigService() as never).execute(interaction);
    expect(reply).toHaveBeenCalledWith({content: 'You need the **Manage Server** permission to use this command.', flags: 64});
  });
});
...
describe('/queue', () => {
  ...
  it('does not expose a phantom page for an exact multiple of upcoming tracks', () => {
    ...
    expect(embed.fields?.find(field => field.name === 'Page')?.value).toBe('1 out of 1');
    expect(() => buildQueueEmbed(player as never, 2, 10)).toThrow('The queue is not that big.');
  });
...
describe('/remove', () => {
  it('rejects a starting position beyond the upcoming queue without replying', async () => {
    ...
    const player = {
      queueSize: () => upcoming.length,
      getQueue: () => upcoming.map(title => ({title, requestedBy: 'requester'})),
    ...
    await expect(new Remove(managerFor(player) as never, fakeConfigService() as never).execute(interaction)).rejects.toThrow('position is outside the range of the queue');
  ...
  it('removes the last valid upcoming position and replies', async () => {
    ...
    await new Remove(managerFor(player) as never, fakeConfigService() as never).execute(interaction);

    expect(upcoming).toEqual(['first']);
    expect(reply).toHaveBeenCalledWith('Removed.');
  });
});
...
describe('/stop', () => {
  for (const [name, status] of Object.entries(STATUS)) {
    it(`stops, disconnects, and clears a connected ${name} session`, async () => {
      ...
      expect(reply).toHaveBeenCalledWith('Stopped playback.');
    });
  }
...
describe('/loop-queue', () => {
  it('accepts current plus one upcoming song', async () => {
    ...
    expect(reply).toHaveBeenCalledWith('Looping queue.');
  });
  ...
  it('rejects when there is no upcoming song', async () => {
    ...
    await expect(new LoopQueue(managerFor(player) as never).execute(interaction)).rejects.toThrow('Not enough songs to loop the queue.');
    expect(reply).not.toHaveBeenCalled();
  });
});
...
describe('/config get', () => {
  it('shows requester-only responses and the voice reduction target from their own settings', async () => {
    mocks.getGuildSettings.mockResolvedValue({
      ...
      volumeDucking: true,
      volumeDuckingTarget: 23,
    });
    const {interaction, editReply} = makeInteraction({subcommand: 'get'});

    await new Config(fakeConfigService() as never).execute(interaction);

    const response = editReply.mock.calls[0][0] as {embeds: Array<{toJSON: () => {description?: string}}>};
    const description = response.embeds[0].toJSON().description;
    expect(description).toContain('**Reduce volume when people speak**: Yes');
