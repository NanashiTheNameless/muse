...
describe('/config set-wait-after-queue-empties', () => {
  it.each([0, 37])('persists the nonnegative delay %i', async delay => {
    const {interaction, editReply} = makeConfigInteraction(delay);

    await new Config({INSTANCE_OWNER_ID: ''} as never).execute(interaction);
    ...
    expect(editReply).toHaveBeenCalledWith('Wait delay updated.');
  });
});
