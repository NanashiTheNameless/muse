...
describe('SQLite persistence', () => {
  // NOTE: upstream's pre-Prisma (Sequelize) migration test was removed: this fork
  // squashed its migration history in 2026 and no longer ships the legacy
  // 20220101155430_migrate_from_sequelize migration it exercises.
...
describe('key-value cache TTL call-site contracts', () => {
  it('uses one hour for search, details, autocomplete, and SponsorBlock, but one minute for playlist reads', async () => {
    ...
    try {
      ...
      const play = new Play(autocompleteCache as never, {} as never, {INSTANCE_OWNER_ID: ''} as never);
