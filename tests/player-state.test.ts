...
describe('Player voice ducking', () => {
  it('ducks while people speak and restores the session volume after the last speaker', () => {
    const player = new Player({} as never, GUILD_ID);
  ...
  it('restores volume when a recorded speaker leaves before their end event', () => {
    const player = new Player({} as never, GUILD_ID);
    const {handlers, members} = installVoiceActivityFakes(player);
    player.status = STATUS.PLAYING;
    player.setVolume(35);
    handlers.get('start')!('speaker-one');
    vi.advanceTimersByTime(400);
...
describe('progress marker bounds', () => {
  it.each([
    ...
    expect(characters.filter(character => character === 'o')).toHaveLength(1);
    expect(progressBar.startsWith('o')).toBe(startsWithMarker);
    expect(progressBar.endsWith('o')).toBe(endsWithMarker);
  });
});
